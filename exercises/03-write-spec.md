...
Turn the intent into a spec an agent cannot misread. Review the agent's plan
before anyone writes code.

## Starting state

`docs/waitlist/intent.md` from Ü2 (or `git stash -u; git checkout checkpoint/02-intent`).
...
4. Anything it got wrong is a spec problem. Fix the spec, leave the prompt.
   Repeat step 3 once.
5. Commit the spec.
...
Save the approved plan as `docs/waitlist/plan.md` and commit it. Then intent,
spec, plan, diff and review sit in git as one audit trail.

## Behind?

`git stash -u; git checkout checkpoint/03-spec`.
