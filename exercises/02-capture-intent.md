...
Turn a one-line idea into a reviewed `docs/waitlist/intent.md` — by letting
the agent interview you instead of prompting it to build.
...
4. Commit it: `git add docs && git commit -m "docs(waitlist): capture intent"`.
...
two intents — every difference is an assumption someone would have made silently.

## Behind?

`git checkout checkpoint/02-intent` — it contains a finished intent.
