...
const run = (payload: unknown) => spawnSync(process.execPath, [hook], { input: JSON.stringify(payload), encoding: 'utf8' }).status;
...
describe('guard-edits hook', () => {
  ...
  it('denies editing an existing test (VS Code)', () => {
    expect(run({ tool_name: 'replace_string_in_file', tool_input: { filePath: 'apps/api/src/server.test.ts' } })).toBe(2);
  });
  ...
  it('denies shell writes to tests, allows running them', () => {
    ...
    expect(run({ tool_name: 'Bash', tool_input: { command: 'grep -n AC3 apps/api/src/waitlist.test.ts 2>&1' } })).toBe(0);
    expect(run({ tool_name: 'Bash', tool_input: { command: 'echo x > apps/api/src/waitlist.test.ts 2>/dev/null' } })).toBe(2);
  ...
  it('judges the target, not the content (a doc may mention package.json)', () => {
    expect(run({ tool_name: 'Edit', tool_input: { file_path: 'README.md', new_string: 'see package.json and x.test.ts' } })).toBe(0);
  });
  it('guards shell commands from PowerShell (Claude Code on Windows) and Cursor', () => {
    expect(run({ tool_name: 'PowerShell', tool_input: { command: 'Set-Content apps/api/src/server.test.ts x' } })).toBe(2);
  ...
  it('reads Codex patches: new test files allowed, existing ones and hook setup denied', () => {
    const patch = (body: string) => ({ tool_name: 'apply_patch', tool_input: { command: `*** Begin Patch\n${body}\n*** End Patch` } });
