...
2. Write one test per acceptance criterion. Put the criterion id in the test
   ...
   its own state (e.g. its own `createApp()`): a test that only passes after
   another one ran is broken, and phase 2 may not fix it. New tests go in a
   new file next to the code.
3. Run only the new tests. Every one must fail **for the right reason**
   (missing behaviour; a typo, import error or wrong URL does not count).
